...
int main(){
  ...
  while(!done){
    ...
    else if (command == "move_assign"){ // the first args value is moved to 2nd arg. 1st arg invalid afterwards
      cin >> which; // reads a b c or d
      char which2;
      cin >> which2; // reads a b c or d
    ...
    else if (command == "move_copy"){ // makes new object with same values but different addresses. 1st arg invalid afterwards
